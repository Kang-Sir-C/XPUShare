...
const (
	logDirectory = "/xpushare/log/"
)

type XPUShareFormatter struct {
}

func (ksf *XPUShareFormatter) Format(entry *logrus.Entry) ([]byte, error) {
	var b *bytes.Buffer
	if entry.Buffer != nil {
		b = entry.Buffer
	} else {
...
func New(level int64, filename string) *logrus.Logger {
	...
	logger.SetFormatter(&XPUShareFormatter{})
