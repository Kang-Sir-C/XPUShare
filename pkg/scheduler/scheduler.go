...
const (
	...
	// the file storing the log of xpushare scheduler
	logPath = "xpushare-scheduler.log"
	// the file storing physical gpu position
	//configPath = "/xpushare/scheduler/xpushare-config.yaml"

	configPath = "/xpushare-topology/xpushare-config.yaml"
...
var (
	_ framework.QueueSortPlugin = &XPUShareScheduler{}
	_ framework.PreFilterPlugin = &XPUShareScheduler{}
	_ framework.FilterPlugin    = &XPUShareScheduler{}
	_ framework.ReservePlugin   = &XPUShareScheduler{}
	_ framework.UnreservePlugin = &XPUShareScheduler{}
	_ framework.ScorePlugin     = &XPUShareScheduler{}
)
...
type Args struct {
	...
	XPUShareConfig string `json:"xpuShareConfig,omitempty"`
...
func New(config *runtime.Unknown, handle framework.FrameworkHandle) (framework.Plugin, error) {
	...
	args := &Args{
		...
		PrometheusURL:                 "http://prometheus-operator-kube-p-prometheus.monitoring:9090",
		XPUShareConfig:               configPath,
	...
	kss := &XPUShareScheduler{
		...
		leafCellsMutex:                &sync.RWMutex{},
		ksl:                           ksl,
		gpuCfg:                        gpu.LoadConfigFromEnv(),
		clock:                         util.RealClock{},
		boundPodQueue:                 map[string]*queue.Queue{},
		boundPodQueueMutex:            &sync.RWMutex{},
	}
	// gpu topology
	xpushareConfig := kss.initRawConfig()
	// ksl.Debugln("=================READ CONFIG=================")
	// ksl.Debugf("%+v", xpushareConfig)
	kss.watchConfig(xpushareConfig)

	// ksl.Debugln("=================CELL ELEMENTS=================")
	ce := kss.buildCellChains(xpushareConfig.CellTypes)
	// for k, v := range ce {
	// 	ksl.Debugf("%+v = %+v", k, v)
	// }
	// ksl.Debugln("=================FREE CELL=================")
	cellFreeList := newCellConstructor(ce, xpushareConfig.Cells, ksl).build()
...
func (kss *XPUShareScheduler) Name() string {
	return Name
}
...
func (kss *XPUShareScheduler) Less(podInfo1, podInfo2 *framework.PodInfo) bool {
...
func (kss *XPUShareScheduler) PreFilter(ctx context.Context, state *framework.CycleState, pod *v1.Pod) *framework.Status {
...
func (kss *XPUShareScheduler) PreFilterExtensions() framework.PreFilterExtensions {
	kss.ksl.Infof("[PreFilterExtensions]")
	return nil
}

// filter the node that doesn't meet the gpu requirements.
func (kss *XPUShareScheduler) Filter(ctx context.Context, state *framework.CycleState, pod *v1.Pod, node *schedulernodeinfo.NodeInfo) *framework.Status {
...
// 2. opportunistic pod
// 3. guarantee pod
func (kss *XPUShareScheduler) Score(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodeName string) (int64, *framework.Status) {
...
func (kss *XPUShareScheduler) ScoreExtensions() framework.ScoreExtensions {
	kss.ksl.Infof("[ScoreExtensions]")
	return kss
}

func (kss *XPUShareScheduler) NormalizeScore(ctx context.Context, state *framework.CycleState, pod *v1.Pod, scores framework.NodeScoreList) *framework.Status {
...
func (kss *XPUShareScheduler) Reserve(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodeName string) *framework.Status {
...
func (kss *XPUShareScheduler) Unreserve(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodeName string) {
...
func (kss *XPUShareScheduler) Permit(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodeName string) (*framework.Status, time.Duration) {
