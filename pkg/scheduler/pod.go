...
const (
	...
	XPUShareLibraryPath = "/xpushare/library"
)
...
func (kss *XPUShareScheduler) addPod(obj interface{}) {
	pod := obj.(*v1.Pod)
...
func (kss *XPUShareScheduler) updatePod(oldObj, newObj interface{}) {
...
func (kss *XPUShareScheduler) deletePod(obj interface{}) {
	pod := obj.(*v1.Pod)
...
func (kss *XPUShareScheduler) filterPod(obj interface{}) bool {
	switch t := obj.(type) {
	case *v1.Pod:
		pod := obj.(*v1.Pod)
...
func (kss *XPUShareScheduler) getPodPrioriy(pod *v1.Pod) (string, bool, int32) {
...
func (kss *XPUShareScheduler) getPodLabels(pod *v1.Pod) (string, bool, *PodStatus) {
...
func (kss *XPUShareScheduler) deletePodStatus(pod *v1.Pod) (*PodStatus, bool) {
	key := fmt.Sprintf("%v/%v", pod.Namespace, pod.Name)
	kss.podStatusMutex.Lock()
	defer kss.podStatusMutex.Unlock()
...
func (kss *XPUShareScheduler) newAssumedMultiGPUPod(pod *v1.Pod, nodeName string) *v1.Pod {
	...
	var cellIDs, uuids, indices, models bytes.Buffer
	totalMemory := int64(0)
	for _, cell := range cells {
		...
		uuids.WriteString(",")
		indices.WriteString(strconv.Itoa(cell.deviceIndex))
		indices.WriteString(",")
		models.WriteString(cell.cellType)
		models.WriteString(",")
	}

	podCopy.Annotations[PodCellID] = strings.TrimRight(cellIDs.String(), ",")
	podCopy.Annotations[PodGPUMemory] = strconv.FormatInt(totalMemory, 10)

	model := strings.TrimRight(models.String(), ",")
	podCopy.Annotations[PodGPUModel] = model
	ps.model = model

	uuid := strings.TrimRight(uuids.String(), ",")
	podCopy.Annotations[PodGPUUUID] = uuid
	podCopy.Annotations[PodGPUIndex] = strings.TrimRight(indices.String(), ",")
	ps.uuid = uuid
	podCopy.ResourceVersion = ""
	podCopy.Spec.NodeName = nodeName
	ps.nodeName = nodeName

	visibleIDs := kss.visibleDevicesValueForCells(cells)
	for i := range podCopy.Spec.Containers {
		c := &podCopy.Spec.Containers[i]
		for _, envName := range kss.gpuCfg.VisibleDevicesEnvVars() {
...
func (kss *XPUShareScheduler) newAssumedSharedGPUPod(pod *v1.Pod, nodeName string) *v1.Pod {
	_, _, ps := kss.getPodLabels(pod)
	ps.uid = ""
	cell := ps.cells[0]
	...
	uuid := cell.uuid
	podCopy.Annotations[PodGPUUUID] = uuid
	podCopy.Annotations[PodGPUIndex] = strconv.Itoa(cell.deviceIndex)
	ps.uuid = uuid
	...
	visibleID := kss.visibleDevicesValueForCell(cell)
	for i := range podCopy.Spec.Containers {
		c := &podCopy.Spec.Containers[i]
		for _, envName := range kss.gpuCfg.VisibleDevicesEnvVars() {
			c.Env = append(c.Env, v1.EnvVar{
		...
		// Only inject the hook library when enabled (default: enabled).
		if kss.gpuCfg.HookEnabled() {
			c.Env = append(c.Env, v1.EnvVar{
				Name:  "LD_PRELOAD",
				Value: XPUShareLibraryPath + "/" + kss.gpuCfg.HookLibraryName,
			})
			c.Env = append(c.Env, v1.EnvVar{
				Name:  "XPUSHARE_ENFORCEMENT_LAYER",
				Value: kss.gpuCfg.EnforcementLayerMode(),
			})
		}
		c.Env = append(c.Env,
			v1.EnvVar{
		...
		//////
		originalMounts := c.VolumeMounts
		c.VolumeMounts = make([]v1.VolumeMount, 0, len(originalMounts)+1)
		c.VolumeMounts = append(c.VolumeMounts, originalMounts...)

		if kss.gpuCfg.HookEnabled() {
			c.VolumeMounts = append(c.VolumeMounts,
				v1.VolumeMount{
					Name:      "xpushare-lib",
					MountPath: XPUShareLibraryPath,
				},
			)
		}
	}
	/////
	if kss.gpuCfg.HookEnabled() {
		originalVolumes := podCopy.Spec.Volumes
		podCopy.Spec.Volumes = make([]v1.Volume, 0, len(originalVolumes)+1)
		podCopy.Spec.Volumes = append(podCopy.Spec.Volumes, originalVolumes...)
		podCopy.Spec.Volumes = append(podCopy.Spec.Volumes,
			v1.Volume{
				Name: "xpushare-lib",
				VolumeSource: v1.VolumeSource{
					HostPath: &v1.HostPathVolumeSource{
						Path: XPUShareLibraryPath,
					},
				},
			},
		)
...
func (kss *XPUShareScheduler) reserveResource(cell *Cell, request float64, memory int64) {
...
func (kss *XPUShareScheduler) reclaimResource(cell *Cell, request float64, memory int64) {
...
func (kss *XPUShareScheduler) processBoundPodQueue(nodeName string) {
...
func (kss *XPUShareScheduler) processBoundPod(pod *v1.Pod) {
...
func (kss *XPUShareScheduler) setPodStatus(pod *v1.Pod, podStatus *PodStatus, request float64, memory int64) {
...
func (kss *XPUShareScheduler) visibleDevicesValueForCell(cell *Cell) string {
	if cell == nil {
		return ""
	}
	switch kss.gpuCfg.VisibleDevicesValueMode() {
	case "index":
		return strconv.Itoa(cell.deviceIndex)
	case "uuid":
		fallthrough
	default:
		return cell.uuid
...
func (kss *XPUShareScheduler) visibleDevicesValueForCells(cells CellList) string {
	if len(cells) == 0 {
		return ""
	}
